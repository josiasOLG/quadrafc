...
export class DashboardService {
  ...
  async getDashboardStats(userId: string) {
    try {
      ...
      const palpitesHoje = await this.palpiteModel.find({
        userId: userId,
        createdAt: {
      ...
      const jogosHoje = await this.jogoModel.find({
        data: {
      ...
      const proximosJogos = await this.jogoModel.find({
        data: { $gte: hoje, $lte: amanha },
        status: 'aberto'
      }).limit(5).sort({ data: 1 }).exec();
      ...
      const usuariosBairro = await this.userModel.find({ 
  ...
  async getJogosDestaque() {
    ...
    return await this.jogoModel.find({
      data: { $gte: hoje, $lte: amanha },
      status: 'aberto'
    }).limit(3).sort({ data: 1 }).exec();
  }

  async getPalpitesRecentes(userId: string, limite: number = 5) {
    return await this.palpiteModel.find({ userId: userId })
      .populate('jogoId')
      .sort({ createdAt: -1 })
      .limit(limite)
      .exec();
  ...
  async getAtividadeBairro(userId: string) {
    const user = await this.userModel.findById(userId).populate('bairroId').exec();
    if (!user || !user.bairroId) return [];

    // Buscar atividade recente do bairro (últimos palpites)
    const atividadeRecente = await this.palpiteModel.find({
      userId: { $in: await this.userModel.find({ bairroId: user.bairroId }).select('_id') }
    })
  ...
  async getMissoesAtivas(userId: string) {
    ...
    const palpitesSemana = await this.palpiteModel.find({
      userId: userId,
      createdAt: {
  ...
  async getProgressoSemanal(userId: string) {
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      const palpitesDia = await this.palpiteModel.find({
        userId: userId,
        createdAt: {
  ...
  async getResumoDiario(userId: string) {
    ...
    return {
      ...
      melhor_palpite: palpitesHoje.length > 0 ? {
